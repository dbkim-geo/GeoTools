...
public class QueryLab extends JFrame {
	...
	public QueryLab() {
		...
		fileMenu.add(new SafeAction("Connect to PostGIS databsae...") {
			public void action(ActionEvent e) throws Throwable {
	...
	private void connect(DataStoreFactorySpi format) throws Exception {
		...
		if (result == JWizard.FINISH) {
			...
			if (dataStore == null) {
				JOptionPane.showMessageDialog(null, "Coul not connect - check parameters");
			}
			updateUI();
		}
	}
	...
	private void filterFeatures() throws Exception{
		//featureSource.getFeatures(filter)를 이용하여 feature data를 갖고옴
